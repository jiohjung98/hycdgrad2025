  ...
  resampleCommands(cmds){
    ...
    for(const c of cmds){
      ...
      if(c.type==='Q'){ const p0=pos, p1={x:c.x1,y:c.y1}, p2={x:c.x,y:c.y};
        const pts=this.sampleQuadAdaptive(p0,p1,p2,this.CURVE_TOLERANCE_PX,0,[]);
        for(const pt of pts) push(pt); pos=p2; continue; }
      if(c.type==='C'){ const p0=pos, p1={x:c.x1,y:c.y1}, p2={x:c.x2,y:c.y2}, p3={x:c.x,y:c.y};
        const pts=this.sampleCubicAdaptive(p0,p1,p2,p3,this.CURVE_TOLERANCE_PX,0,[]);
        for(const pt of pts) push(pt); pos=p3; continue; }
  ...
  async loadSVG(url){
    try{
      ...
      for(const node of doc.querySelectorAll('path')){
        ...
        const cmds=this.parsePathD(d); const subs=this.resampleCommands(cmds);
    ...
    for(const path of this.paths){
      for(const sp of path.subpaths){
        for(const p of sp.points){
          ...
          const ax=fxS+fxR+fxD, ay=fyS+fyR+fyD;
  ...
  render(){
    ...
    for(const path of this.paths){
      ...
      for(const sp of path.subpaths){
        ...
        for(let i=1;i<sp.points.length;i++){ const q=sp.points[i]; ctx.lineTo(q.x, q.y); }
    ...
    if (THEME.art3.strokeWeight > 0) {
      ...
      for(const path of this.paths){
        for(const sp of path.subpaths){
          ...
          if(sp.closed) endShape(CLOSE); else endShape();
        }
      }
    }
  }
...
function setup() {
  ...
  window.__toggleArt3 = (on = !A3.enabled) => { A3.enabled = !!on; };
}
